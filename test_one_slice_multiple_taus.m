...
name     = fname;
tol_smooth_array = [0.01,0.02,0.04,0.06,0.09,0.10,0.20,0.30];
...
for j=1:size(tol_smooth_array,2)
    tau_val = tol_smooth_array(j);
    param_struct.tol_smooth = tau_val;
    ...
    sgtitle(strjoin(["mask mean=",param_struct.l2_mean,"tau = ",tau_val, "rho= ", rho],' '));
