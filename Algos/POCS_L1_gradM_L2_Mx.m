...
smooth_max(1) = sum(abs( MGradxS(:) - param_struct.l2_mean_grad ),'all' )  ;
...
%     v4 = v4_ - sigma4 * proj_l2ball( sigma4^(-1) * v4_, param_struct.l2_bound, param_struct.l2_mean ) ;
    ...
    v4 = v4_ - sigma4 * (oneProjector( (sigma4^(-1)*v4_)-param_struct.l2_mean_grad, param_struct.l2_bound_grad)  + param_struct.l2_mean_grad);
...
%     l2smooth(it+1) = max(abs( MGradxS - param_struct.l2_mean )) ;
    ...
    smooth_max(it+1) = sum(abs( MGradxS(:) - param_struct.l2_mean_grad ),'all' )  ;
...
%     
    ...
    if mod(it, param_algo.display) == 0
        ...
        disp(['      vs. upper bound    = ',num2str(param_struct.l2_bound_pix)])
        disp('---------------------------------------------')
        disp(['|| grad (MxS)-mean_grad ||_1                = ',num2str(smooth_max(it+1))])
        disp(['      vs. upper bound    = ',num2str(param_struct.l2_bound_grad)])
        disp('*********************************************')
